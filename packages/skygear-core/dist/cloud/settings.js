...
function loadHttpSettings() {
  ...
  var enabled = process.env.HTTP || true;
  var httpAddr = process.env.HTTP_ADDR || '0.0.0.0:9000';

  var addrComponents = httpAddr.split(':', 2);
  if (addrComponents.length < 2) {
    addrComponents.unshift('0.0.0.0');
  }

  var address = addrComponents[0] ? addrComponents[0] : '0.0.0.0';
  var port = parseInt(addrComponents[1]);
...
function parseInteger(val, defaultValue) {
  if (val === '' || val === null || val === undefined) {
    return defaultValue;
  }
...
var settings = exports.settings = {
  ...
  pgPoolIdleTimeoutMillis: parseInteger(process.env.PG_POOL_IDLE_TIMEOUT_MILLIS, 3000),

  assetStore: process.env.ASSET_STORE || 'fs',

  // FS asset settings
  assetStoreSecret: process.env.ASSET_STORE_SECRET || 'asset-secret',
  assetStoreURLPrefix: process.env.ASSET_STORE_URL_PREFIX || 'http://127.0.0.1:3000/files',
  assetStoreURLExpireDuration: parseInteger(process.env.ASSET_STORE_URL_EXPIRE_DURATION, 3600),
